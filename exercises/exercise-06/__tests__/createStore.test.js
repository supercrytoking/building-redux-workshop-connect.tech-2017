...
test('second parameter function, is used to create a createStore function, accepting the reducer', () => {
    const reducer = getMockReducer();
    const internalCreateStore = jest.fn();
    const augmentCreateStore = jest.fn()
        .mockReturnValue(internalCreateStore);
    const store = createStore(reducer, augmentCreateStore);
